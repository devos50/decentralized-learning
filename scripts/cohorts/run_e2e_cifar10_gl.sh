...
python3 -u simulations/gl/cifar10.py --peers 200 --duration 72100 --bypass-model-transfers --seed $SEED --capability-trace data/client_device_capacity --accuracy-logging-interval 3600 --partitioner dirichlet --alpha $ALPHA --cohort-file "cohorts/cohorts_n200_c${COHORTS}_s${SEED}_a${ALPHA}.txt" --compute-validation-loss-global-model --local-steps 5 --gl-round-timeout 10 --train-device-name "cuda:0" --accuracy-device-name "cuda:0" --log-level "ERROR" > output_c${COHORTS}_s${SEED}_a${ALPHA}_p${PARTICIPATION}.log 2>&1
