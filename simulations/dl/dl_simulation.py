...
class DLSimulation(LearningSimulation):
    ...
    def __init__(self, args: Namespace) -> None:
        ...
        self.round_durations: List[Tuple[int, float]] = []
    ...
    def flush_statistics(self):
        """
        Flush all the statistics generated by nodes and coordinator.
        """
        super().flush_statistics()
        with open(os.path.join(self.data_dir, "round_durations.csv"), "w") as out_file:
            for round_nr, duration in self.round_durations:
                out_file.write("%d,%f\n" % (round_nr, duration))
    ...
    async def on_round_complete(self, peer_ind: int, round_nr: int):
        ...
        self.num_round_completed = 0
        self.round_durations.append((round_nr, get_event_loop().time() - self.round_start_time))
