...
class DataPartitioner(object):
    ...
    def use(self, partition, istest):
        ...
        exeuteLength = len(resultIndex) if not istest else int(len(resultIndex))
...
def select_dataset(rank, partition, batch_size, isTest=False, collate_fn=None):
    ...
    partition = partition.use(rank, isTest)
    dropLast = False if isTest else True
    if isTest:
        num_loaders = 0
    else:
        num_loaders = min(int(len(partition)/batch_size/2), 0)
    if num_loaders == 0:
        time_out = 0
    else:
        time_out = 60
