...
python3 create_cohort_file.py 200 $COHORTS --seed $SEED --partitioner dirichlet --alpha 0.1 --output "../data/cohorts/cohorts_n200_c${COHORTS}_s${SEED}_data.txt"

python3 -u simulations/dfl/cifar10.py --peers 200 --duration 0 --num-aggregators 1 --activity-log-interval 60 --bypass-model-transfers --seed $SEED --capability-trace data/client_device_capacity --accuracy-logging-interval 0 --partitioner dirichlet --alpha 0.1 --fix-aggregator --cohort-file "cohorts/cohorts_n200_c${COHORTS}_s${SEED}_data.txt" --compute-validation-loss-global-model --train-device-name "cuda:0" --accuracy-device-name "cuda:0" > output_c${COHORTS}_s${SEED}.log 2>&1
